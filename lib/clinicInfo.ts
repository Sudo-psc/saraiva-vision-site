/**
 * Clinic Information Module
 * Centralized clinic data with environment variable resolution
 */
...
const normalizePlaceId = (value: string | undefined): string | null => {
...
const resolvePlaceId = (): string => {
  const candidates: (string | undefined)[] = [];

  // Next.js environment variables
  if (typeof process !== 'undefined' && process.env) {
    candidates.push(process.env.GOOGLE_PLACE_ID);
    candidates.push(process.env.NEXT_PUBLIC_GOOGLE_PLACE_ID);
...
export const clinicInfo = {
  ...
  validateSchedulingUrl: (): string | null => {
