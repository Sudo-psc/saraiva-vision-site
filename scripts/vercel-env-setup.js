...
function setEnvVar(name, value, environments) {
    const envFlags = environments.map(env => `--environment ${env}`).join(' ');
    const command = `env add ${name} ${envFlags}`;

    console.log(`Setting ${name} for environments: ${environments.join(', ')}`);

    // Use echo to pipe the value to avoid shell escaping issues
    try {
        execSync(`echo "${value}" | vercel env add ${name} ${envFlags}`, {
            encoding: 'utf8',
            stdio: 'inherit'
        });
