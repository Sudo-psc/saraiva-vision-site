...
/**
 * TypeScript interfaces for Services component
 */
...
/**
 * ServiceCard component - Individual service card with animations and lazy loading
 */
...
const Services: React.FC<ServicesProps> = ({ full = false, autoplay = true }) => {
  ...
  const loadServices = useCallback(async (): Promise<void> => {
    setLoading(true);
    const clinicServices: ServiceItem[] = [
  ...
  const cardWidthRef = useRef<number>(320); // width + gap
  const itemsPerViewRef = useRef<number>(1);
  ...
  const autoplayCarousel: AutoplayHook = useAutoplayCarousel({
    ...
    onSlideChange: useCallback((newIndex: number, direction: string) => {
  ...
  const canScrollFurther = useCallback((el: HTMLDivElement, deltaX: number): boolean => {
    if (!el) return false;
    return deltaX > 0
  ...
  const updateIndex = useCallback((): number => {
    // Simplified to prevent loops - just calculate, don't update
    const el = scrollerRef.current;
    if (!el || serviceItems.length === 0) return 0;
  ...
  const scrollByAmount = useCallback((dir = 1): void => {
    const el = scrollerRef.current;
    if (!el) return;
  ...
  const onPointerDown = useCallback((e: React.PointerEvent | React.TouchEvent) => {
    ...
    const isTouch = 'touches' in e || (e as React.PointerEvent).pointerType === 'touch';

    setIsDragging(true);
    autoplayCarousel.pause(); // Use hook's pause method
    dragStartXRef.current = 'touches' in e ? e.touches[0].clientX : (e as React.PointerEvent).clientX || 0;
    scrollStartRef.current = el.scrollLeft;

    // Apenas capture pointer para mouse, não para touch
    if (!isTouch && (el as any).setPointerCapture) {
      try { (el as any).setPointerCapture((e as React.PointerEvent).pointerId); } catch (_) { }
    }
  }, [autoplayCarousel]);
  ...
  const onPointerMove = useCallback((e: React.PointerEvent | React.TouchEvent) => {
    ...
    const clientX = 'touches' in e ? e.touches[0].clientX : (e as React.PointerEvent).clientX || 0;
