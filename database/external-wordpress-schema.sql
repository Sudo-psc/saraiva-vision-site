...
CREATE TABLE IF NOT EXISTS external_wordpress_content (
    ...
    INDEX idx_content_compliance (source_id, compliance_score, status)
);
...
CREATE TABLE IF NOT EXISTS external_wordpress_sync_logs (
    ...
    -- Indexes
    INDEX idx_source_sync (source_id),
    INDEX idx_action_status (action, status),
    INDEX idx_started_at (started_at),
    INDEX idx_completed_at (completed_at),
    INDEX idx_error_code (error_code),
    INDEX idx_sync_performance (duration, items_processed)
);
...
RETURNS TABLE (source_id BIGINT, name VARCHAR, sync_frequency VARCHAR, is_valid BOOLEAN) AS $$
BEGIN
    RETURN QUERY
    SELECT
        s.id,
