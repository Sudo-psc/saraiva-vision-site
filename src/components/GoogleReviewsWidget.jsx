    ...
    const {
        ...
        onError: (err) => {
            if (err.message.includes('not configured')) {
                console.debug('[Google Reviews Debug] Not configured, using fallback data:', err.message);
            } else {
                console.debug('[Google Reviews Debug] API error, using fallback data:', err.message);
            }
        }
    });
    ...
    return (
        <section className={`py-12 bg-white ${className}`}>
            <div className="max-w-7xl mx-auto px-[7%]">
                ...
                {/* Status Indicators - Hidden from UI, kept for debugging */}
                {loading && (
                ...
                {/* Development Debug Info - Console Only */}
                {process.env.NODE_ENV === 'development' && (
                    <div className="sr-only" aria-hidden="true">
                        {/* This will trigger debug logging without showing in UI */}
                        {(() => {
                            if (isUsingFallback) {
                                console.debug('[Google Reviews Debug] Using fallback data');
                            }
                            if (error) {
                                console.debug('[Google Reviews Debug] Error:', error.message);
                            }
                            return null;
                        })()}
                    </div>
