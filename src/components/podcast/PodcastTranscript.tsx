...
const PodcastTranscript: React.FC<PodcastTranscriptProps> = ({ episode }) => {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-2 border-gray-200 overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-gray-500 to-cyan-600 px-6 py-4">
      ...
      {/* Summary */}
      <div className="px-6 py-4 bg-gradient-to-br from-gray-50 to-cyan-50">
        <h4 className="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Resumo</h4>
        ...
        <div className="px-6 py-4 border-t border-gray-200">
          ...
          <div className="flex flex-wrap gap-2">
            {keywords.map((keyword, index) => (
              <span
                key={index}
                className="px-3 py-1 bg-gradient-to-r from-gray-100 to-cyan-100 text-cyan-700 rounded-full text-sm font-medium hover:from-gray-200 hover:to-cyan-200 transition-colors cursor-default"
              >
                {keyword}
              </span>
            ))}
          ...
          >
            <div className="px-6 py-4 space-y-4">
              {highlights.map((highlight, index) => (
                <div
                  key={index}
                  className="flex gap-4 p-4 bg-gradient-to-r from-gray-50 to-cyan-50 rounded-xl border border-gray-200"
                >
                  <div className="flex-shrink-0">
                    <div className="w-16 h-16 bg-gradient-to-br from-gray-500 to-cyan-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg">
                      {highlight.timestamp}
                    </div>
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {highlight.keywords.map((kw, i) => (
                        <span
                          key={i}
                          className="px-2 py-1 bg-white text-cyan-700 rounded-md text-xs font-medium border border-cyan-200"
                        >
                          {kw}
                        </span>
                      ))}
            ...
            <div className="px-6 py-6 bg-white">
              <div
                className="prose prose-lg max-w-none
                  prose-headings:text-cyan-700 prose-headings:font-bold
                  prose-h3:text-xl prose-h3:mt-8 prose-h3:mb-4
                  prose-h4:text-lg prose-h4:mt-6 prose-h4:mb-3
                  prose-p:text-gray-700 prose-p:leading-relaxed prose-p:mb-4
                  prose-strong:text-cyan-800 prose-strong:font-semibold
      ...
      {episode.relatedServices && episode.relatedServices.length > 0 && (
        <div className="px-6 py-4 border-t border-gray-200 bg-gradient-to-br from-gray-50 to-cyan-50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {episode.relatedServices.map((service, index) => (
              <Link
                key={index}
                to={service.url}
                className="flex items-center gap-3 px-4 py-3 bg-white hover:bg-cyan-50 rounded-xl border border-gray-200 hover:border-cyan-400 transition-all duration-300 group shadow-sm hover:shadow-md"
              >
                <div className="p-2 bg-gradient-to-br from-gray-100 to-cyan-100 group-hover:from-gray-200 group-hover:to-cyan-200 rounded-lg transition-colors">
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-white">
          ...
          <div className="space-y-2">
            {episode.relatedPosts.map((post, index) => (
              ...
              >
                <svg className="w-5 h-5 text-cyan-500 group-hover:text-cyan-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
